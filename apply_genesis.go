...
func ApplyGenesis(statedb *state.StateDB, g ncogearthchain.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g ncogearthchain.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g ncogearthchain.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
