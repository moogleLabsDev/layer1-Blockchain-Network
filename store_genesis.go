...
func (s *Store) GetRules() ncogearthchain.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &ncogearthchain.Rules{}).(*ncogearthchain.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg ncogearthchain.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() ncogearthchain.Genesis {
	meatadata := s.GetMetadata()
	return ncogearthchain.Genesis{
